---
name: childrens-kdp-book-creator
description: Creates original children’s books, tracing books, coloring books, alphabet books, number books, and activity books with consistent image-generated artwork.
---


# Children’s KDP Book Creator

## Purpose

Create age-appropriate, original children’s products with clear educational or entertainment value.
...
- Matching, counting, and observation books

## Workflow

1. Define age range, reading level, learning goal, page count, trim size, and theme.
2. Create an age-appropriate page plan or story outline.
3. Establish a consistent character and illustration guide.
4. Generate the actual illustrations, activity pages, title pages, and cover using ChatGPT Image Generation 2.0.
5. Keep characters, proportions, clothing, colors, and environments consistent.
6. Avoid overcrowding and ensure instructions are simple and readable.
7. Check that activities are solvable and age appropriate.
8. Create parent-facing listing copy and educational benefit callouts.

## Additional safety

Do not create content that sexualizes minors, uses unsafe challenges, or imitates protected children’s characters or franchises.


## Non-negotiable creation rules

1. **Create the product, not only the idea.** Do not stop after brainstorming, outlining, or writing prompts. Continue through the usable product workflow.
2. **Use ChatGPT Image Generation 2.0 for visual creation.** Whenever the task involves covers, interior pages, worksheets, activity pages, illustrations, decorative elements, dividers, title pages, mockups, or other visual assets, use ChatGPT Image Generation 2.0 when it is available.
3. **Do not merely describe an image.** Generate the visual asset in the chat when image generation is available. Prompts may be shown as supporting documentation, but they are not a substitute for creating the image.
4. **Preserve consistency.** Maintain the approved theme, characters, palette, typography direction, trim size, margins, bleed choice, visual hierarchy, and recurring page system across the product.
5. **Support the customer’s requested size.** Accept any KDP-compatible trim size or custom size requested by the user. Do not force a default size when the user supplies one.
6. **Create in manageable batches.** When a book requires many pages, create and approve a master style and several representative pages first, then continue in organized batches while maintaining the same design system.
7. **Use original content.** Do not copy copyrighted books, brand artwork, trademarked characters, celebrity likenesses, or another creator’s exact layout. References may guide structure or mood only.
8. **KDP production awareness.** Before final delivery, check trim size, bleed, safe margins, page count, spine requirements, image resolution, color mode expectations, and current Amazon KDP publishing requirements.
9. **Quality control is required.** Check spelling, page numbering, repeated-page consistency, clipped text, low-resolution assets, awkward spacing, visual crowding, duplicated content, and missing sections.
10. **No-code workflow.** Do not require the user to code. Use ChatGPT, image generation, Canva, document tools, or other no-code production tools.
